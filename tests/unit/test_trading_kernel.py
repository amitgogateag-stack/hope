...
def make_signal():
    return Signal(signal_id=uuid4(), instrument_id=uuid4(), strategy_version="s1", decision_time=datetime.now(timezone.utc), signal_type=SignalType.ENTRY, conviction=Decimal("0.8"), inputs_hash="a"*64)
...
def test_approved_risk_preserves_signal_identity():
    signal = make_signal()
