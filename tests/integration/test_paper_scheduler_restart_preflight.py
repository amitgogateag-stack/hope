...
def test_failed_prior_run_does_not_block_fresh_due_run() -> None:
    ...
    failed = create_scheduled_job_run(
        "paper-batch-failed",
        now - timedelta(minutes=2),
    )
    fresh = create_scheduled_job_run(
        "paper-batch-fresh-after-failure",
        now - timedelta(minutes=1),
    )
    calls = []
    ...
    with engine.begin() as connection:
        ...
        assert repository.complete(
            create_job_run_completion(
                failed,
                JobRunStatus.FAILED,
                failed.scheduled_for + timedelta(seconds=30),
                failure_code="EXPECTED_PAPER_FAILURE",
            )
        ) is True
    ...
    registry = PaperJobRegistry(
        [
            PaperJobDefinition(
                failed.job_key,
                lambda runtime: calls.append(failed.job_run_id),
            ),
            PaperJobDefinition(
                fresh.job_key,
                lambda runtime: calls.append(fresh.job_run_id),
...
def test_stale_terminal_history_does_not_block_fresh_due_work() -> None:
    ...
    terminal = create_scheduled_job_run(
        "paper-batch-stale-terminal",
        now - timedelta(hours=2),
    )
    fresh = create_scheduled_job_run(
        "paper-batch-fresh-after-stale-terminal",
        now - timedelta(minutes=1),
    )
    calls = []
