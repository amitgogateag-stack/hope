...
def test_fill_cumulative_quantity_cannot_exceed_source_order() -> None:
    ...
    with engine.connect() as connection:
        ...
        try:
            ...
            signal_id = uuid4()
            order_id = uuid4()
            fill_time = datetime(2026, 9, 12, 22, 45, tzinfo=timezone.utc)

            connection.execute(
                text("INSERT INTO instruments(instrument_id, canonical_symbol, exchange, status) VALUES (:id, 'FILL-CUMULATIVE-INTEGRITY', 'TEST', 'ACTIVE')"),
                {"id": instrument_id},
            )
            connection.execute(
